...
def get_weather(location: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "humidity": f"{humidity}%",
            "wind_speeds_mps": f"{wind_speed} m/s",
        }
...
def compare_weather_prompt(location_a: str, location_b: str) -> str:
    ...
    return f"""
    You are acting as a helpful weather analyst. Your goal is to provide a clear and eas-to-read comparison of the weather in two different locations for a user.
...
def delivery_log_resource() -> list[str]:
    ...
    try:
        ...
        # Read the file, remove leading/trailing whitespaces, and split into lines
        return log_file.read_text(encoding="utf-8").strip().splitlines()
    except Exception as e:
        return [f"An unexpected error ocurred while reading the delivery log: {str(e)}"]
