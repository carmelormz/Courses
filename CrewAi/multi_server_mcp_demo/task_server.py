...
def add_task(task_description: str) -> str:
    ...
    try:
        ...
        return f"An error ocurred while adding the task: {e}"
...
def list_tasks() -> List[str]:
    ...
    is emtpty, it returns an empty list.
    ...
    if not os.path.exists(TASKS_FILE):
        # Return an empty list if the file doesn't exists, as there are no tasks
        # The LLM can interpret this as "no tasks to show"
        return []

    try:
        with open(TASKS_FILE, "r") as f:
            # Read all lines, and strip leading/trailing whitespaces (like newlines)
...
def meeting_notes_resource() -> list[str]:
    ...
    try:
        ...
        if not notes_file.exists():
            return ["Error: The meeting_notes.txt file was not found in the server."]

        return notes_file.read_text(encoding="utf-8").strip().splitlines()

    except Exception as e:
        return [
            f"An unexpected error ocurred while reading the meeting notes: {str(e)}"
        ]
